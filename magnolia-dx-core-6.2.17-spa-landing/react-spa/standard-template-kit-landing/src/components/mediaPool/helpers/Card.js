...
const Card = ({ fields, buttonProps }) => {
  ...
  const assetVariantsCount = fields.variantsCount.value;
  const assetTagsArr = fields?.structuredKeywords?.items?.map(item => item.fields.name.value);
  const assetTags = assetTagsArr?.join(', ');
  const assetThemes = fields?.themes?.items?.map((item, index) => (
    <React.Fragment key={item.fields.id.value}>
      <br />
      {item.fields.text.value}
      {index < fields?.themes?.items?.length - 1 && <br />}
    </React.Fragment>
  ));
  ...
  const internalLinkPath = `${baseURL}${apiBase}/Landing/Search-Pages/MP-Search?query=${assetId}`;
  ...
  return (
    <div className='assetCard' onMouseEnter={imageMouseEnter} onMouseLeave={imageMouseLeave}>
      ...
      <div className='assetCardContent'>
        ...
        <p><span>Varianten: </span>{assetVariantsCount}</p> 
        <p><span>Eigentümer: </span>{owner}</p>
        {/* <p><span>File Size:</span>{fileSize}</p>
        <p><span>VDB:</span>{vdb}</p>
        <p><span>Upload Date:</span>{uploadDate}</p> */}
        <p><span>Letzte Änderung: </span>{lastUpdatedTime}</p>            
        {/* {keywords && 
